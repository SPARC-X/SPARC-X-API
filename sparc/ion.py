...
def read_ion(fileobj, recover_constraints=True):  # ,index):
    text = fileobj.read()
    comments_removed = []
    comments = []
    ...
    for comment in comments:
        ...
        if 'PBC:' in comment:
            ...
            del pbc_list, pbc
    # find the index of all the different atom types
...
def write_ion(fileobj, atoms, pseudo_dir = None, scaled = True,
    ...
    for comp in atoms.cell:
        fileobj.write('  {}'.format(format(np.linalg.norm(comp) /  Bohr, ' .16f')))
    fileobj.write('\n#LATVEC\n')
    for comp in atoms.cell:
        fileobj.write('#')
        comp_n = comp / np.linalg.norm(comp)  # normalize
        for i in comp_n:
            fileobj.write(' {}'.format(format(i, ' .16f')))
        fileobj.write('\n')
        ...
        if pseudo_dir is not None:
            if not os.path.isdir(pseudo_dir):
                ...
                warnings.warn('The path entered for `pseudo_dir` could '
                              'not be found, using SPARC_PSP_PATH instead')
        ...
        for atom, position in zip(atoms, positions):
            if atom.symbol == element:
                for component in position:
                    fileobj.write('    {}'.format(format(component, ' .16f')))
